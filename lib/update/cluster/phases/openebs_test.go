...
type PhaseOpenEBSSuite struct {
	backend   storage.Backend
	cluster   storage.Site
	backupDir string
}

var _ = check.Suite(&PhaseOpenEBSSuite{})

func (s *PhaseOpenEBSSuite) SetUpSuite(c *check.C) {
...
func (s *PhaseOpenEBSSuite) TestUpgradeVolumes(c *check.C) {
	phase, err := NewPhaseUpgradeVolumes(nil, logrus.StandardLogger())
	c.Assert(err, check.IsNil)
...
func (s *PhaseOpenEBSSuite) TestMigrateRoles(c *check.C) {
...
func (s *PhaseOpenEBSSuite) TestMigrateLinks(c *check.C) {
